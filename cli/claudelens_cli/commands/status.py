...
@click.command()
@click.option(
    "--detailed", "-d", is_flag=True, help="Show detailed project-level statistics"
)
def status(detailed: bool):
    ...
    if server_status["connected"]:
        status_info.append(
            f"[green]✓[/green] Server: Connected ({config_manager.config.api_url})"
        )
        status_info.append(f"  Version: {server_status.get('version', 'Unknown')}")
    else:
        status_info.append(
            f"[red]✗[/red] Server: {server_status.get('error', 'Not connected')}"
        )
