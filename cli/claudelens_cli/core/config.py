...
class CLIConfig(BaseModel):
    ...
    api_url: str = Field(
        default="http://localhost:8000", description="ClaudeLens API URL"
    )
    ...
    sync_interval: int = Field(
        default=300, description="Sync interval in seconds for watch mode"
    )
    batch_size: int = Field(
        default=100, description="Number of messages to sync in one batch"
    )
    log_level: str = Field(default="INFO", description="Logging level")
    sync_types: list[str] = Field(
        default=["projects", "todos", "database"], description="Data types to sync"
    )
    exclude_projects: list[str] = Field(
        default_factory=list, description="Project paths to exclude from sync"
    )
...
class ConfigManager:
    ...
    def _load_config(self) -> CLIConfig:
        ...
        if claude_dir := os.getenv("CLAUDE_DIR"):
            ...
            if "," in claude_dir:
                env_config["claude_dirs"] = [
                    Path(d.strip()) for d in claude_dir.split(",")
                ]
            else:
            ...
            env_config["claude_dirs"] = [
                Path(d.strip()) for d in claude_dirs.split(",")
            ]
    ...
    def save(self) -> None:
        ...
        with open(self.config_file, "w") as f:
            json.dump(self.config.model_dump(mode="json"), f, indent=2, default=str)
