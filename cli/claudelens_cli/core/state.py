        ...
        if self.state_file.exists():
            try:
                ...
                for project_data in data.get("projects", {}).values():
                    ...
                    project_data["synced_sessions"] = set(project_data.get("synced_sessions", []))
                    project_data["synced_messages"] = set(project_data.get("synced_messages", []))
                if db_state := data.get("database_state"):
                    db_state["last_sync"] = datetime.fromisoformat(db_state["last_sync"])
                    if db_state.get("last_message_timestamp"):
                        db_state["last_message_timestamp"] = datetime.fromisoformat(db_state["last_message_timestamp"])
