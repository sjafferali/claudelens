...
class MessageService:
    ...
    async def list_messages(
        ...
        # Get messages
        cursor = self.db.messages.find(filter_dict).sort(
            "timestamp", sort_direction
    ...
    async def get_message_context(
        ...
        # Get messages before
        before_cursor = self.db.messages.find({
            "sessionId": session_id,
            "timestamp": {"$lt": timestamp}
        }).sort("timestamp", -1).limit(before)
        ...
        # Get messages after
        after_cursor = self.db.messages.find({
            "sessionId": session_id,
            "timestamp": {"$gt": timestamp}
        }).sort("timestamp", 1).limit(after)
